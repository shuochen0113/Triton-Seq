# benchmarking/scripts/benchmark_extz_ptx_0820.py
"""
...
from utils.io_OPv4 import read_fasta_as_bytes
from utils.packing_kernel_OPv4 import pack_batch_into_buffers
...
from framework_api_OPv4 import run_alignment_task
...
PTX_PATH = project_root / "ptx_modification_0804" / "sw_kernel_int16_hack_v2.ptx"
TRITON_PTX_PATH = project_root / "ptx_modification_0804" / "sw_kernel_int16.ptx"
FUNC_NAME = b"sw_kernel"  # PTX .entry name
NUM_PAIRS = 65536

# --- Tile wavefront PTX/func constants ---
PTX_TW_PATH = project_root / "ptx_modification_0804" / "sw_tw_cuda_banded.ptx"
TW_FUNC_NAME = b"sw_tile_wavefront_int16_banded"
...
# --- Helpers for tile-wavefront kernel ---
def concat_to_u8_device(seq_list):
    """
    Concatenate a list of Python bytes objects into a single torch.uint8 CUDA tensor,
    and build int32 offsets/lengths (CUDA side will index into the flat buffer).
    Returns (cat_u8, off_i32, len_i32).
    """
    n = len(seq_list)
    lens_np = np.fromiter((len(s) for s in seq_list), dtype=np.int32, count=n)
    offs_np = np.zeros(n, dtype=np.int32)
    if n > 1:
        np.cumsum(lens_np[:-1], out=offs_np[1:])
    # Build one big bytes buffer
    buf = bytearray(int(lens_np.sum()))
    pos = 0
    for s in seq_list:
        ln = len(s)
        buf[pos:pos+ln] = s
        pos += ln
    # cat_np = np.frombuffer(bytes(buf), dtype=np.uint8)
    cat_np = np.frombuffer(bytes(buf), dtype=np.uint8).copy()  # 加 .copy()，得到可写连续数组
    cat = torch.from_numpy(cat_np).to(device="cuda", non_blocking=True)
    # cat = torch.from_numpy(cat_np).to(device="cuda", non_blocking=True)
    off = torch.from_numpy(offs_np).to(device="cuda", non_blocking=True)
    ln  = torch.from_numpy(lens_np).to(device="cuda", non_blocking=True)
    return cat, off, ln

def compute_shmem_bytes_tw(TW: int = 128, PAD_PER_32: int = 1) -> int:
    """
    Dynamic shared memory size used by sw_tile_wavefront_int16_banded:
    Hprev/Hcurr/Eprev (int16, padded) + tiny halos (int16) + sref tile (uint8).
    """
    Wpad = TW + (TW // 32) * PAD_PER_32
    # Hprev (Wpad*2) + Hcurr (Wpad*2) + Eprev (Wpad*2) + halos (2*(TW//32)*2) + sref (TW + 16)
    shmem_bytes = (Wpad * 3 + 2 * (TW // 32)) * 2 + (TW + 16)
    return int((shmem_bytes + 15) // 16 * 16)  # align to 16B
...
def run_ptx_tw_alignment(q_list, r_list, *,
                         ptx_path=PTX_TW_PATH,
                         func_name: bytes = TW_FUNC_NAME,
                         TW_val: int = 128,
                         PAD_PER_32_val: int = 1,
                         label: str = "[PTX CUDA TW]",
                         carveout: int = -1):
    """
    Launch the tile-wavefront CUDA PTX kernel (sw_tile_wavefront_int16_banded).
    Returns (outs_cpu[N,3], kernel_ms, cells_static, metrics_dict).
    """
    assert len(q_list) == len(r_list)
    batch_size = len(q_list)
    # 1) Build flat uint8 buffers + i32 offsets/lengths (device)
    q_base, q_off, q_len = concat_to_u8_device(q_list)
    r_base, r_off, r_len = concat_to_u8_device(r_list)

    # 2) Allocate outputs (device)
    out_max_score = torch.empty(batch_size, dtype=torch.int32, device="cuda")
    out_max_i     = torch.empty_like(out_max_score)
    out_max_j     = torch.empty_like(out_max_score)

    # Workspace for cross-tile carries (size = sum of all q lengths)
    total_q = int(q_len.sum().item())
    carryH = torch.zeros(total_q, dtype=torch.int16, device="cuda")
    carryF = torch.zeros_like(carryH)

    # 3) Shared memory configuration (very small)
    shared_bytes = compute_shmem_bytes_tw(TW=TW_val, PAD_PER_32=PAD_PER_32_val)

    # 4) Load PTX + function
    ensure_cuda_context()
    mod, fn = load_ptx_function(ptx_path, func_name, maxrreg=0, carveout=carveout)

    # Allow dynamic shared opt-in (harmless if not supported)
    try:
    ...
    # 5) Kernel arguments (must match .entry)
    # Scoring/band/zdrop: keep identical to baseline config
    args_np = [
        np.array([q_base.data_ptr()], dtype=np.uint64),
        np.array([r_base.data_ptr()], dtype=np.uint64),
        np.array([q_off.data_ptr()],  dtype=np.uint64),
        np.array([r_off.data_ptr()],  dtype=np.uint64),
        np.array([q_len.data_ptr()],  dtype=np.uint64),
        np.array([r_len.data_ptr()],  dtype=np.uint64),
        np.array([np.int16(CONFIG['match'])],    dtype=np.int16),
        np.array([np.int16(CONFIG['mismatch'])], dtype=np.int16),
        np.array([np.int16(CONFIG['gap_open'])], dtype=np.int16),
        np.array([np.int16(CONFIG['gap_extend'])], dtype=np.int16),
        np.array([np.int32(CONFIG['band_width'])], dtype=np.int32),       # band_half
        np.array([np.int32(0)], dtype=np.int32),                          # band_center_shift
        np.array([np.int32(CONFIG['z_drop'])], dtype=np.int32),
        np.array([out_max_score.data_ptr()], dtype=np.uint64),
        np.array([out_max_i.data_ptr()],     dtype=np.uint64),
        np.array([out_max_j.data_ptr()],     dtype=np.uint64),
        np.array([carryH.data_ptr()], dtype=np.uint64),
        np.array([carryF.data_ptr()], dtype=np.uint64),
    ]
    ...
    # 6) Grid/block
    block_x = 128  # must match .reqntid in PTX
    grid_x  = batch_size
    ...
    # 8) Gather outputs as Nx3 for the same checker
    outs_cpu = np.stack([
        out_max_score.cpu().numpy(),
        out_max_i.cpu().numpy(),
        out_max_j.cpu().numpy()
    ], axis=1)

    # Static-band cells (for GCUPS)
    cells_static = 0
    for m_i, n_i in zip(q_len.tolist(), r_len.tolist()):
        cells_static += diag_band_cells(int(m_i), int(n_i), CONFIG['band_width'])
...
def run_ptx_alignment(q_list, r_list, *, ptx_path=PTX_PATH, use_shared: bool = True,
    ...
    shared_bytes_formula = ((header_bytes + 3 * STRIDE * 2 + 15) // 16) * 16
...
def main():
    ...
    q_all = read_fasta_as_bytes(dataset_path / "query.fa")
    r_all = read_fasta_as_bytes(dataset_path / "ref.fa")
    ...
    with open(csv_path, "w", newline="") as fcsv:
        ...
        for npairs in sweep_sizes:
            ...
            # --- PTX (CUDA tile-wavefront) ---
            tw_outs, tw_k_ms, tw_cells, m_tw = run_ptx_tw_alignment(q_list, r_list,
                                                                     ptx_path=PTX_TW_PATH,
                                                                     func_name=TW_FUNC_NAME,
                                                                     TW_val=128,
                                                                     PAD_PER_32_val=1,
                                                                     label="[PTX CUDA TW]",
                                                                     carveout=-1)
            print(f"[PTX CUDA TW]   kernel_ms={tw_k_ms:.3f} (static-band cells={tw_cells/1e9:.3f} G)")
            print(f"[PTX CUDA TW]   GCUPS={gcups_from_cells(tw_cells, tw_k_ms):.3f}")
            ...
            check_and_maybe_dump(tw_outs, "PTX(CUDA-TW)")
            ...
            # --- Write CSV rows ---
            writer.writerow([npairs, m_tw["label"], f"{tw_k_ms:.3f}", f"{gcups_from_cells(tw_cells, tw_k_ms):.3f}",
                             m_tw.get("active_blocks_sm"), m_tw.get("dyn_smem_used"), m_tw.get("regs"),
                             m_tw.get("shared_static"), m_tw.get("local_bytes"), m_tw.get("max_threads_per_block"), m_tw.get("reqntid"), m_tw.get("ptx")])
