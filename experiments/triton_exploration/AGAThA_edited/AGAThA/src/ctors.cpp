...
void gasal_destroy_streams(gasal_gpu_storage_v *gpu_storage_vec, Parameters *params) {
	...
	for (i = 0; i < gpu_storage_vec->n; i ++) {
		...
		if (gpu_storage_vec->a[i].host_res->traceback != NULL) free(gpu_storage_vec->a[i].host_res->traceback);
        if (gpu_storage_vec->a[i].host_res->aligned_query != NULL) free(gpu_storage_vec->a[i].host_res->aligned_query);
        if (gpu_storage_vec->a[i].host_res->aligned_target != NULL) free(gpu_storage_vec->a[i].host_res->aligned_target);

        if (gpu_storage_vec->a[i].device_res->traceback != NULL) CHECKCUDAERROR(cudaFree(gpu_storage_vec->a[i].device_res->traceback));
        if (gpu_storage_vec->a[i].device_res->aligned_query != NULL) CHECKCUDAERROR(cudaFree(gpu_storage_vec->a[i].device_res->aligned_query));
        if (gpu_storage_vec->a[i].device_res->aligned_target != NULL) CHECKCUDAERROR(cudaFree(gpu_storage_vec->a[i].device_res->aligned_target));
