...
void reconstruct_sequence(uint8_t *traceback, char *aligned_query, char *aligned_target, int length);
...
struct gasal_res {
    int32_t *aln_score;  // Alignment score
    int32_t *query_batch_end;  // End position in query sequence
    int32_t *target_batch_end; // End position in target sequence
    int32_t *query_batch_start; // Start position in query sequence
    int32_t *target_batch_start; // Start position in target sequence
    uint8_t *cigar;  // CIGAR string for alignment representation
    uint32_t *n_cigar_ops;  // Number of operations in CIGAR

    // New fields for traceback storage
    uint8_t *traceback;  // Traceback matrix for reconstructing alignment
    char *aligned_query;  // Aligned query sequence
    char *aligned_target; // Aligned target sequence
};
