...
#define CORE_LOCAL_DEPRECATED_COMPUTE() do { \
...
#define CORE_COMPUTE() do { \
    uint32_t rbase = (packed_ref_literal >> l) & 15; \
    DEV_GET_SUB_SCORE_GLOBAL(temp_score, qbase, rbase); \
    temp_score += p[m]; \
    if (temp_score >= f[m] && temp_score >= e) { \
        h[m] = temp_score; \
        traceback_res[ref_idx * query_len + query_idx] = 0; /* Diagonal: match/mismatch */ \
    } else if (f[m] >= e) { \
        h[m] = f[m]; \
        traceback_res[ref_idx * query_len + query_idx] = 1; /* Left: gap in query */ \
    } else { \
        h[m] = e; \
        traceback_res[ref_idx * query_len + query_idx] = 2; /* Up: gap in reference */ \
    } \
...
#define CORE_COMPUTE_BOUNDARY() do { \
...
__global__ void agatha_kernel(uint32_t *packed_query_batch,
                              ...
                              uint8_t *traceback_res)
{
    ...
    const int real_warp_id = threadIdx.x % 32;
    const int warp_per_kernel = (gridDim.x * blockDim.x) / const_warp_len;
    const int job_per_warp = n_tasks % warp_per_kernel ? (n_tasks / warp_per_kernel + 1) : n_tasks / warp_per_kernel;
    const int job_per_query = max_query_len % const_warp_len ? (max_query_len / const_warp_len + 1) : max_query_len / const_warp_len;
    const int job_start_idx = (tid / const_warp_len) * job_per_warp;
    const int job_end_idx = (job_start_idx + job_per_warp) < n_tasks ? (job_start_idx + job_per_warp) : n_tasks;
	...
	for (job_idx = job_start_idx; job_idx < job_end_idx; job_idx++) {
		...
		if (warp_id == 0) {
            device_res->aln_score[ub_idx] = max_score;
            device_res->query_batch_end[ub_idx] = max_query_idx;
            device_res->target_batch_end[ub_idx] = max_ref_idx;
            device_res->traceback_offset[ub_idx] = ub_idx * max_query_len * max_ref_len; // Store traceback location
        }
...
__global__ void agatha_sort(uint32_t *packed_query_batch,
	uint32_t *packed_ref_batch,
	uint32_t *query_batch_lens,
	uint32_t *target_batch_lens,
	uint32_t *query_batch_offsets,
