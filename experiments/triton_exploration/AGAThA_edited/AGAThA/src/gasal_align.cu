...
inline void agatha_kernel_launcher(int32_t kernel_block_num,
                                    int32_t kernel_thread_num,
                                    gasal_gpu_storage_t *gpu_storage,
                                    int32_t actual_n_alns)
{
    /* Sort for Uneven Bucketing */
    agatha_sort<<<kernel_block_num, kernel_thread_num, 0, gpu_storage->str>>>(
        ...
        actual_n_alns,
        gpu_storage->maximum_sequence_length,
        gpu_storage->global_buffer);
        
    cudaMemcpyAsync((void*)(gpu_storage->host_buffer),
                    (const void*)(gpu_storage->global_buffer + kernel_block_num * (kernel_thread_num / 8) * (gpu_storage->maximum_sequence_length) * 3),
                    actual_n_alns * sizeof(uint32_t),
                    cudaMemcpyDeviceToHost,
                    gpu_storage->str);
    cudaStreamSynchronize(gpu_storage->str);
    std::sort(gpu_storage->host_buffer,
              gpu_storage->host_buffer + actual_n_alns,
              [](short2 a, short2 b){ return a.x < b.x; });
    cudaMemcpyAsync((void*)(gpu_storage->global_buffer + kernel_block_num * (kernel_thread_num / 8) * (gpu_storage->maximum_sequence_length) * 3),
                    (const void*)(gpu_storage->host_buffer),
                    actual_n_alns * sizeof(uint32_t),
                    cudaMemcpyHostToDevice,
                    gpu_storage->str);
    
    // Updated kernel launch: pass maximum_sequence_length for both max_query_len and max_ref_len.
	agatha_kernel<<<kernel_block_num, kernel_thread_num, shared_mem, gpu_storage->str>>>(
		gpu_storage->packed_query_batch,
		gpu_storage->packed_target_batch,
		gpu_storage->query_batch_lens,
		gpu_storage->target_batch_lens,
		...
		actual_n_alns,
		gpu_storage->maximum_sequence_length,  // max_query_len
		gpu_storage->maximum_sequence_length,  // max_ref_len
		gpu_storage->global_buffer,
		gpu_storage->device_res->traceback);    // Pass the traceback pointer
}
...
void gasal_aln_async(gasal_gpu_storage_t *gpu_storage, const uint32_t actual_query_batch_bytes, const uint32_t actual_target_batch_bytes, const uint32_t actual_n_alns, Parameters *params) {
	...
	if (gpu_storage->gpu_max_n_alns < actual_n_alns) {
		...
		// Allocate memory for traceback data on host and device
		gpu_storage->host_res->traceback = (uint8_t *)malloc(actual_n_alns * gpu_storage->maximum_sequence_length * sizeof(uint8_t));
		CHECKCUDAERROR(cudaMalloc(&(gpu_storage->device_cpy->traceback), actual_n_alns * gpu_storage->maximum_sequence_length * sizeof(uint8_t)));
		gpu_storage->host_res->aligned_query = (char *)malloc(actual_n_alns * gpu_storage->maximum_sequence_length * sizeof(char));
		gpu_storage->host_res->aligned_target = (char *)malloc(actual_n_alns * gpu_storage->maximum_sequence_length * sizeof(char));
		CHECKCUDAERROR(cudaMalloc(&(gpu_storage->device_cpy->aligned_query), actual_n_alns * gpu_storage->maximum_sequence_length * sizeof(char)));
		CHECKCUDAERROR(cudaMalloc(&(gpu_storage->device_cpy->aligned_target), actual_n_alns * gpu_storage->maximum_sequence_length * sizeof(char)));
	...
	if (gpu_storage->host_res->traceback != NULL && gpu_storage->device_cpy->traceback != NULL)
		CHECKCUDAERROR(cudaMemcpyAsync(gpu_storage->host_res->traceback, gpu_storage->device_cpy->traceback, actual_n_alns * gpu_storage->maximum_sequence_length * sizeof(uint8_t), cudaMemcpyDeviceToHost, gpu_storage->str));
	
	if (gpu_storage->host_res->aligned_query != NULL && gpu_storage->device_cpy->aligned_query != NULL)
		CHECKCUDAERROR(cudaMemcpyAsync(gpu_storage->host_res->aligned_query, gpu_storage->device_cpy->aligned_query,
									   actual_n_alns * gpu_storage->maximum_sequence_length * sizeof(char),
									   cudaMemcpyDeviceToHost, gpu_storage->str));
	
	if (gpu_storage->host_res->aligned_target != NULL && gpu_storage->device_cpy->aligned_target != NULL)
		CHECKCUDAERROR(cudaMemcpyAsync(gpu_storage->host_res->aligned_target, gpu_storage->device_cpy->aligned_target,
									   actual_n_alns * gpu_storage->maximum_sequence_length * sizeof(char),
									   cudaMemcpyDeviceToHost, gpu_storage->str));
	//-----------------------------------------------------------------------------------------------------------------------
	
	size_t traceback_size = actual_n_alns * gpu_storage->maximum_sequence_length * gpu_storage->maximum_sequence_length * sizeof(uint8_t);
	cudaMemcpyAsync(gpu_storage->host_res->traceback, gpu_storage->device_res->traceback, traceback_size, cudaMemcpyDeviceToHost, gpu_storage->str);
	cudaStreamSynchronize(gpu_storage->str);

	for (int i = 0; i < actual_n_alns; i++) {
		// Assume you have stored query and target sequences for alignment i
		reconstruct_sequence(gpu_storage->host_res->traceback + i * (max_query_len * max_ref_len),
							   gpu_storage->host_res->aligned_query + i * (max_query_len + 1),
							   gpu_storage->host_res->aligned_target + i * (max_ref_len + 1),
							   gpu_storage->query_batch_lens[i],
							   gpu_storage->target_batch_lens[i]);
	}
	
	// Output aligned sequences reconstructed from traceback
	// if (params->print_out) {
	// 	for (int i = 0; i < actual_n_alns; i++) {
	// 		reconstruct_sequence(gpu_storage->host_res->traceback + i * gpu_storage->maximum_sequence_length,
	// 							gpu_storage->host_res->aligned_query + i * gpu_storage->maximum_sequence_length,
	// 							gpu_storage->host_res->aligned_target + i * gpu_storage->maximum_sequence_length,
	// 							gpu_storage->maximum_sequence_length);

	// 		params->raw_file << "Query: " << (gpu_storage->host_res->aligned_query + i * gpu_storage->maximum_sequence_length) << std::endl;
	// 		params->raw_file << "Target: " << (gpu_storage->host_res->aligned_target + i * gpu_storage->maximum_sequence_length) << std::endl;
	// 	}
	// }
