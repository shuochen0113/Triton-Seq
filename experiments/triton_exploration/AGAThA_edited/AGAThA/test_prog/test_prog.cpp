				...
				if (gasal_is_aln_async_done(gpu_batch_arr[gpu_batch_arr_idx].gpu_storage) == 0) {
					...
					if(print_out) {
						...
						for (int i = gpu_batch_arr[gpu_batch_arr_idx].batch_start; j < gpu_batch_arr[gpu_batch_arr_idx].n_seqs_batch; i++, j++) {
							...
							std::cout << "\taligned_query=" << (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res->aligned_query[j];
							std::cout << "\taligned_target=" << (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res->aligned_target[j];
