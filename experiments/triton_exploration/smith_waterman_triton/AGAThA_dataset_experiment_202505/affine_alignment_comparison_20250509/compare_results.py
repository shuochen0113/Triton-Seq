...
triton_json_path = "../outputs/guided_affine_alignments_gpu_20250507_012704.json"
agatha_scorelog_path = "/root/shuochen/Cornell_intern/triton_experiment/AGAThA/output/score.log"
output_report_path = "./affine_my_dataset_comparison_report_newest2.txt"
...
for i, (triton_item, agatha_item) in enumerate(zip(triton_data, agatha_data)):
    ...
    if score_match and position_match:
        ...
        # diffs.append((i, t_score, t_qpos, t_rpos, a_score, a_qpos, a_rpos, reason, q_len, r_len))
...
with open(output_report_path, "w") as f:
    ...
    for pid, t_s, t_q, t_r, a_s, a_q, a_r, reason, q_len, r_len in diffs:
        f.write(f"Pair #{pid}: {reason}\n")
        f.write(f"  Triton → score: {t_s}, pos: ({t_q}, {t_r}), query_len: {q_len}, ref_len: {r_len}\n")
