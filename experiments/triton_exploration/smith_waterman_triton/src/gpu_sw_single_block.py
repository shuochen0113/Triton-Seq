...
def sw_kernel_single_block(
    ...
    """
    Single-launch Smith-Waterman kernel that loops over all diagonals.
...
def smith_waterman_gpu_single_block(seq1, seq2, match=3, mismatch=-2, gap=-1):
    ...
    (Pointer arithmetic is done in 64-bit to prevent overflow on large datasets.)
    """
    ...
    torch.cuda.synchronize()
    kernel_time = start.elapsed_time(end)  # ms
    print(f"[Triton Unpacked Timing] Kernel time: {kernel_time:.3f} ms")
