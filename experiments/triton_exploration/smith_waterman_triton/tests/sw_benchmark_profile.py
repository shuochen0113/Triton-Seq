...
sys.path.append(os.path.join(smith_waterman_triton, "src"))

from optimize_experiment import gpu_sw_multi_block, gpu_sw_single_block, gpu_sw_basic, gpu_sw_single_block_packed
...
def run_experiment(seq1, seq2):
    ...
    gpu_start = time.time()
    gpu_score, gpu_pos, gpu_dp = gpu_sw_single_block.smith_waterman_gpu_single_block(seq1, seq2)
    # gpu_score, gpu_pos, gpu_dp = gpu_sw_single_block_packed.smith_waterman_gpu_single_block_packed(seq1, seq2)
    # gpu_score, gpu_pos, gpu_dp = gpu_sw_multi_block.smith_waterman_gpu_multi_block(seq1, seq2)
    # gpu_score, gpu_pos, gpu_dp = gpu_sw_basic.smith_waterman_gpu_basic(seq1, seq2)
    # torch.cuda.synchronize
...
def main():
    reference_path = "/root/shuochen/Cornell_intern/triton_experiment/smith_waterman_triton/dataset/ref_profile1.fasta"
    query_path = "/root/shuochen/Cornell_intern/triton_experiment/smith_waterman_triton/dataset/query_profile1.fasta"
    ...
    for (ref_id, ref_seq), (query_id, query_seq) in zip(reference_sequences.items(), query_sequences.items()):
        ...
        # change this line
        print(f"Finish running Smith-Waterman Single Launch on reference {ref_id} and query {query_id}.", flush=True)
    ...
    # save results
    output_dir = Path("profile/outputs")
    output_dir.mkdir(exist_ok=True)
    # change this line
    results_path = output_dir / f"single_block_results_{timestamp}.json"
    ...
    # change this line
    summary_path = output_dir / f"single_block_summary_{timestamp}.json"
