...
int main(int argc, char **argv) {
	...
	*/
	...
	// ======================== 2025-07-23: accurate time-measurement of multi-batch ========================
    // to calculate total time after the loop
    double total_malloc_time_ms = 0;
    double total_local_launch_time_ms = 0;
    // ================================================
	...
	#pragma omp parallel
	{
		...
		// ======================== 2025-07-23 ========================
		// create a local timer for each thread to measure the time 
        Timer malloc_timer_local;
        Timer launch_timer_local;
        // ================================================
	...
	malloc_time_1.Stop();
	total_malloc_time_ms += malloc_time_1.GetTime();
	...
	if (n_seqs > 0) {
		while (n_batchs_done < thread_n_batchs[omp_get_thread_num()]) { // Loop on streams
			...
			if (seqs_done < n_seqs && gpu_batch_arr_idx < gpu_storage_vecs[omp_get_thread_num()].n) {
				...
				// ======================== 2025-07-23 ========================
                    malloc_timer_local.Stop();
                    #pragma omp critical
                    {
                        total_malloc_time_ms += malloc_timer_local.GetTime();
                    }
                // ================================================
				...
				// cudaStreamSynchronize(gpu_storage_vecs[omp_get_thread_num()].str);  // <—
				// ======================== 2025-07-23 ========================
				// local_time.Stop();
                    launch_timer_local.Stop();
                    #pragma omp critical
                    {
                        total_local_launch_time_ms += launch_timer_local.GetTime();
                    }
                // ================================================
	...
	*/
	...
	fprintf(stderr, "process time (Triton comparable wall time): %.3f\n", process_time.GetTime());

	// fprintf(stderr, "Triton-comparable total time (in milliseconds): %.3f\n", total_malloc_time_ms + post_time.GetTime() + total_local_launch_time_ms);
	// fprintf(stderr, "malloc time (in milliseconds): %.3f\n", malloc_time.GetTime());
	// fprintf(stderr, "free time (in milliseconds): %.3f\n", free_time.GetTime());
	// fprintf(stderr, "malloc time (in milliseconds): %.3f\n", malloc_time.GetTime());
	// ======================== 2025-07-23 ========================
    fprintf(stderr, "h2d+malloc time: %.3f\n", total_malloc_time_ms);
    fprintf(stderr, "local kernel time: %.3f\n", total_local_launch_time_ms);
    // ================================================
	fprintf(stderr, "post time (in milliseconds): %.3f\n", post_time.GetTime());
