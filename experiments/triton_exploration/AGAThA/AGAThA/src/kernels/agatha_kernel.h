...
//=========2025-07-14:AGAThA Ablation Study=============
#define ENABLE_UB 1
#define ENABLE_SR 1
#define ENABLE_RW 1
//===========================
...
// ==========2025-07-14: ablation study============
#define CORE_COMPUTE_NO_RW() \
        uint32_t rbase = (packed_ref_literal >> l) & 15;\
        DEV_GET_SUB_SCORE_GLOBAL(temp_score, qbase, rbase) \
        temp_score += p[m]; \
        h[m] = max(temp_score, f[m]); \
        ...
        e = max(temp_score- _cudaGapOE, e - _cudaGapExtend); \
        p[m] = h[m-1]; \
        int global_diag_idx = (ub_idx * (max_query_len * 2)) + (ref_idx + m - 1 + query_idx); \
        atomicMax(&device_antidiag_max[global_diag_idx], (h[m]<<16) +ref_idx+ m-1);
// ==================================================
...
// ==========2025-07-14: ablation study============
#define CORE_COMPUTE_BOUNDARY_NO_RW() \
		if (query_idx + _cudaBandWidth < ref_idx + m-1 || query_idx - _cudaBandWidth > ref_idx + m-1) { \
			p[m] = h[m-1]; \
		} else { \
			uint32_t rbase = (packed_ref_literal >> l) & 15;\
			...
			e = max(temp_score- _cudaGapOE, e - _cudaGapExtend); \
			p[m] = h[m-1]; \
			/* if no RW, use atomic operations to update global mem */ \
			int global_diag_idx = (ub_idx * (max_query_len * 2)) + (ref_idx + m - 1 + query_idx); \
			atomicMax(&device_antidiag_max[global_diag_idx], (h[m]<<16) +ref_idx+ m-1); \
		}
// ==================================================
...
// ============2025-07-14: ablation study (for RW)==============
...
__global__ void agatha_kernel(uint32_t *packed_query_batch, uint32_t *packed_ref_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, gasal_res_t *device_res_second, uint4 *packed_tb_matrices, int n_tasks, uint32_t max_query_len, short2 *global_buffer_top, int32_t* device_antidiag_max)
// =======================
{
	...
	/* Setup values that will change after Subwarp Rejoining */
		...
		if (warp_id==0) shared_job[(warp_num&3)] = -1;
	#else
		// no SR，warp_len and warp_id are always const
		const int warp_len = const_warp_len;
		const int warp_id = threadIdx.x % warp_len;
		int warp_num = tid / warp_len;
		unsigned same_threads = __match_any_sync(0xffffffff, warp_num);	
	#endif
	//==================================================
	...
	for (job_idx = job_start_idx; job_idx < job_end_idx; job_idx++) {
		...
		ub_idx = ((job_idx&3)==0)? global_ub_idx[n_tasks-(job_idx>>2)-1].y: global_ub_idx[job_idx-(job_idx>>2)-1].y;
		#else
		// If UB is disabled, we simply process tasks in their original, sequential order.
		ub_idx = job_idx; 
		#endif
		//================================
		...
		while (i < total_anti_diags) {
			...
			while (!terminated && finished_blocks <= slice_end) {
				...
				if (active) {
					ref_idx = packed_ref_idx << 3; // multiply 8 (get real idx)
					...
					packed_ref_literal = packed_ref_batch[packed_ref_batch_idx + packed_ref_idx]; // load 8 references
				}
				...
				for (y = 0; y < _cudaSliceWidth; y++) {
					if (active && chunk_start <= packed_query_idx && packed_query_idx <= chunk_end) {
						...
						packed_query_literal = packed_query_batch[packed_query_batch_idx + packed_query_idx];  // load 8 query
						query_idx = packed_query_idx << 3;
						
						// Bitwise operation to read the 8 bases in each 32 bit
			...
			if (!terminated) {
				for (diag_idx = i<<3; diag_idx < last_diag; diag_idx++) {
					if (diag_idx <prev_max_score) {
						...
						// =========== 2025-07-14: ablation study =========
						#else
                        // no RW, read max from global mem
                        int global_diag_idx = (ub_idx * (max_query_len * 2)) + diag_idx;
                        temp = device_antidiag_max[global_diag_idx];
						#endif
						// =========================================

						if ((temp>>16) > max_score) {				
							max_score = temp>>16; // In agatha, score is stored in higher 16 bit. Notice that anitidiag_max lower 16 bit is used to store reference coordinate!!!!
			...
			if (i >= total_anti_diags) {
				...
				if (!terminated) {
					...
					for (k = i*packed_len, m = diag_idx; m < diag_idx+packed_len; m++, k++) {
						...
						#else
                            // no RW, read max from global mem
                            int global_diag_idx = (ub_idx * (max_query_len * 2)) + k;
                            temp = device_antidiag_max[global_diag_idx];
                        #endif
                        // ======================================
						if ((temp>>16) > max_score) {				
