...
void gasal_init_streams(gasal_gpu_storage_v *gpu_storage_vec,  int max_query_len, int max_target_len, int32_t maximum_sequence_length, Parameters *params) {
	...
	for (i = 0; i < gpu_storage_vec->n; i++) {
		...
		//  malloc global mem for "no RW"
		// size: every task * max anti-diag length, it's safe
		int max_diag_len = maximum_sequence_length * 2; 
		CHECKCUDAERROR(cudaMalloc(&(gpu_storage_vec->a[i].device_antidiag_max), (long)max_n_alns * max_diag_len * sizeof(int32_t)));
		// init it as INT_MIN, it's important for atomicMax
		CHECKCUDAERROR(cudaMemsetAsync(gpu_storage_vec->a[i].device_antidiag_max, 0x80, (long)max_n_alns * max_diag_len * sizeof(int32_t), gpu_storage_vec->a[i].str));
		// ======================================
...
void gasal_destroy_streams(gasal_gpu_storage_v *gpu_storage_vec, Parameters *params) {
	...
	for (i = 0; i < gpu_storage_vec->n; i ++) {
		...
		// ===================2025-07-14: ablation study ===================
		if (gpu_storage_vec->a[i].device_antidiag_max != NULL) CHECKCUDAERROR(cudaFree(gpu_storage_vec->a[i].device_antidiag_max));
		// ======================================
