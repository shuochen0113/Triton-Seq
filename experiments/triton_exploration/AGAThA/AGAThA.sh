...
DATASET_DIR="/root/autodl-tmp/Cornell_intern/prototype_20250621/datasets/"
FINAL_DIR="$PWD"
...
while [ "$iter" -lt "$ITER" ]; do  
    ...
    # ${PROG_DIR}manual -p -m 1 -x 4 -q 6 -r 2 -s 3 -z 400 -w 751 ${DATASET_DIR}ref_x75.fa ${DATASET_DIR}query_x75.fa ${RAW_FILE} > ${SCORE_FILE}
    ${PROG_DIR}manual -p -m 1 -x 4 -q 6 -r 2 -s 3 -z 400 -w 751 ${DATASET_DIR}ref.fa ${DATASET_DIR}query.fa ${RAW_FILE} > ${SCORE_FILE}
    ((iter++))
    sleep "${IDLE}s"
done
