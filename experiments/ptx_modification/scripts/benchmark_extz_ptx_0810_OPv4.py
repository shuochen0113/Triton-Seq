...
from utils.packing_kernel_OPv3 import pack_batch_into_buffers
from host.buffer_manager_OPv3 import create_pipeline_resources
...
PTX_PATH = project_root / "ptx_modification_0804" / "ptx" / "sw_kernel_hack_v1.ptx"
TRITON_PTX_PATH = project_root / "ptx_modification_0804" / "ptx" / "sw_kernel.ptx"
...
def run_ptx_alignment(q_list, r_list, *, ptx_path=PTX_PATH, use_shared: bool = True,
    ...
    header_bytes = 64
    STRIDE = 2 * band_width + 1
    # unified formula (7*STRIDE*4 + header, 16B alignment)
    ...
    # Occupancy
    try:
        active_blocks = cuda.cuOccupancyMaxActiveBlocksPerMultiprocessor(fn, block_x, shared_bytes)[1]
        try:
            sms = cuda.cuDeviceGetAttribute(_device_attr(cuda.CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT), cuda.cuDeviceGet(0)[1])[1]
        except Exception:
            sms = torch.cuda.get_device_properties(0).multi_processor_count
        print(f"[Occu {label}] active_blocks/SM={active_blocks}, total_blocks_theoretical={active_blocks * sms}")
    except Exception as e:
        print(f"[Occu {label}] (skip) {e}")

    try:
        dev = cuda.cuDeviceGet(0)[1]
        smem_per_blk_optin = cuda.cuDeviceGetAttribute(_device_attr( __import__('builtins').getattr(cuda, 'CU_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_BLOCK_OPTIN', 97) ), dev)[1]
        print(f"[Device {label}] max_dyn_smem/block_optin={smem_per_blk_optin}B, dyn_smem_used={shared_bytes}B")
    except Exception as e:
    ...
    metrics = {
        ...
        "active_blocks_sm": int(active_blocks) if 'active_blocks' in locals() else None,
        "smem_optin": int(smem_per_blk_optin) if 'smem_per_blk_optin' in locals() else None,
