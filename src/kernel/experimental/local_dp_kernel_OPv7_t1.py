...
2025-08-26
 ...
 OPv7 (Aggressive Inlining):
    - Final attempt to reduce register pressure by aggressively inlining address calculations.
    - Removes intermediate vector variables for slot addresses (e.g., slot_H_curr, slot_e)
      and computes them directly inside memory operation calls.
    - This gives the compiler maximum freedom for register allocation and instruction scheduling.
"""
...
def sw_kernel(
    ...
    while (d <= m + n) & (stop == 0):
        ...
        while off < L:
            ...
            # --- ADDRESSING & DP (WITH AGGRESSIVE INLINING) ---
            
            # H-Buffer Addresses (Inlined)
            slot_H_curr = CENTER + t_vec
            
            # E/F Buffer Addresses (Inlined)
            slot_e_start = CENTER + t0 + R_E
            slot_e_start = tl.where(slot_e_start >= STRIDE, slot_e_start - STRIDE, slot_e_start)
            thr_e = STRIDE - slot_e_start
            slot_e = slot_e_start + tid
            slot_e = tl.where(tid >= thr_e, slot_e - STRIDE, slot_e)

            slot_f_start = CENTER + t0 + R_F
            slot_f_start = tl.where(slot_f_start < 0, slot_f_start + STRIDE, slot_f_start)
            slot_f_start = tl.where(slot_f_start >= STRIDE, slot_f_start - STRIDE, slot_f_start)
            thr_f = STRIDE - slot_f_start
            slot_f = slot_f_start + tid
            slot_f = tl.where(tid >= thr_f, slot_f - STRIDE, slot_f)

            # --- Data Loading, DP, and Storage ---
